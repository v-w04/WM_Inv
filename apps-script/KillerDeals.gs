 ...
 *  cientos de items son ~4-6 llamadas por refresco, y solo se
 *  refresca cada KD_REFRESH_MIN minutos: las ofertas no cambian
 *  cada 15 minutos como el stock.
 */

/** Cada cuánto se vuelve a bajar. Las ofertas duran días, no minutos. */
const KD_REFRESH_MIN = 180;

/** Llave de PropertiesService con el timestamp del último refresco */
 ...
 */
function fetchIncentivos_(status, deadline) {
  const out = [];
...
function sincronizarKillerDeals(deadline) {
  ...
  const elegibles = fetchIncentivos_('ELIGIBLE', lim);
  const inscritos = fetchIncentivos_('ENROLLED', lim);
  ...
  PropertiesService.getScriptProperties()
    .setProperty(KD_PROP_ULTIMO, String(Date.now()));
...
/** ¿Ya toca refrescar las ofertas? */
function kdTocaRefrescar_() {
  const last = Number(PropertiesService.getScriptProperties()
                        .getProperty(KD_PROP_ULTIMO) || 0);
  if (!last) return true;
  return (Date.now() - last) / 60000 >= KD_REFRESH_MIN;
}
...
function refrescarKillerDeals() {
  const r = sincronizarKillerDeals();
  const L = [];
  ...
  L.push('');
  L.push('Listo en ' + r.elapsedSec + 's.');
